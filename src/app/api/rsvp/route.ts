...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { 'CO/DE': coDe, code, name, email, whatsapp, rsvp, '+1': plusOne, '19-Connect': connect19, '21-Boat': boat21, notes, song } = parsedData.data;

    // Persist to Notion
    try {
      console.log('Attempting to add or update RSVP to Notion:', { 'CO/DE': coDe, code, name, email, whatsapp, rsvp: rsvp || 'Yes', '+1': plusOne, '19-Connect': connect19, '21-Boat': boat21, notes, song });
      await addRSVP({ 'CO/DE': coDe, code, name, email, whatsapp, rsvp: rsvp || 'Yes', '+1': plusOne, '19-Connect': connect19, '21-Boat': boat21, notes, song });
