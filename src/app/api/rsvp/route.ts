...
export async function POST(req: NextRequest) {
  ...
  const { code, name, whatsapp, rsvp, '19-Connect': connect19, 'BigDay': bigDay, '21-Boat': boat21, notes, song } = parsedData.data;

    // Persist to Notion
    try {
  console.log('Attempting to add or update RSVP to Notion:', { code, name, whatsapp, rsvp: rsvp || 'Yes', '19-Connect': connect19, 'BigDay': bigDay, '21-Boat': boat21, notes, song });
  await addRSVP({ code, name, whatsapp, rsvp: rsvp || 'Yes', '19-Connect': connect19, 'BigDay': bigDay, '21-Boat': boat21, notes, song });
