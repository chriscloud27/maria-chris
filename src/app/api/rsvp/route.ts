...
export async function POST(req: NextRequest) {
  ...
  const { code, name, whatsapp, '+1': plusOne, '19-Connect': connect19, 'BigDay': bigDay, '21-Boat': boat21, notes, song } = parsedData.data;

    // Persist to Notion
    try {
  console.log('Attempting to add or update RSVP to Notion:', { code, name, whatsapp, '+1': plusOne, '19-Connect': connect19, 'BigDay': bigDay, '21-Boat': boat21, notes, song });
  await addRSVP({ code, name, whatsapp, '+1': plusOne, '19-Connect': connect19, 'BigDay': bigDay, '21-Boat': boat21, notes, song });
